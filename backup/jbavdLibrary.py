...
class LoadDataFromDatabaseThread(QtCore.QThread):
    ...
    def __init__(self, items: list, container: object, parent=None):
        ...
        try:
            ...
            self.general_funcion = GeneralFunctions()
        except Exception as e:
        ...
        try:
            ...
            for item in self.item_list:
                if str(item[11]).lower() != 'deleted':
                    m_data = self.general_funcion.database_list_to_dictionary(item)
...
class AddNewDownloadThread(QtCore.QThread):
    ...
    def stop(self):
        ...
        try:
            ...
            # self.any_signal.emit(self.data)
            print("[] AddNewDownloadThread stopped...")
    ...
    def run(self):
        ...
        try:
            ...
            if url_exists is False:
                ...
                self.data['message'] = "checkig itereet"
                self.any_signal.emit(self.data)
                time.sleep(3)
                while True:
                    if self.is_running is False:
                        ...
                        self.data['message'] = "self is runnig"
                        self.any_signal.emit(self.data)

                    self.data['message'] = "checking internet from general function"
                    self.any_signal.emit(self.data)
                    time.sleep(4)
                    try:
                        ans = self.general_functions.IsInternet()
                        self.data['message'] = f"result from general function.isInternet: {ans}"
                        self.any_signal.emit(self.data)
                        time.sleep(4)
                        if ans is True:
                            break
                    ...
                    self.data['message'] = f'Internet issue detected. Retrying. Please wait...{ans}'
                    self.any_signal.emit(self.data)
                    time.sleep(5)

                self.data['message'] = "checkig if url cotai playlist"
                self.any_signal.emit(self.data)
                time.sleep(3)
                try:
    ...
    def run(self):
        ...
        try:
            try:
                ...
                url = self.container.textURL.text()
                print(f"url: {url}")
                # isPlaylist = GeneralFunctions.is_playlist(GeneralFunctions, url)
...
class LoadItemDataThread(QtCore.QThread):
    ...
    def run(self):
        ...
        try:
            self.my_parent.progressBar.setValue(0)

            self.my_parent.textTitle.setText(self.title)
            self.my_parent.textStatus.setText(self.status)

            self.display_content()

            self.my_parent.dataLoadingCompleted = True
